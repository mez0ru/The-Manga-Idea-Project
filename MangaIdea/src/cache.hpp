// Github: https://github.com/vpetrigo/caches

/* 
    Slightly modified to accomedate value pointers,
    if it erases a value to store a new one, it has to call the destructor of the object.
*/
...
class fixed_sized_cache
{
  ...
  protected:
    ...
    void Erase(const_iterator elem)
    {
        ...
        // if the value is pointer, then call the destructor.
        if (std::is_pointer<Value>::value)
        {
            delete cache_items_map[elem->first];

            #ifdef _DEBUG
            fmt::print("Called destructor\n");
            #endif
        }
